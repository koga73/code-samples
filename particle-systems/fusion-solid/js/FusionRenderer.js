...
var FusionRenderer = (function(){
    ...
    var _consts = {
		...
		FOREGROUND_ALPHA:0x62 / 0xFF,
		PARTICLE_SPEED:5 / _pixelRatio,
		PARTICLE_SPEED_RANGE:20 / _pixelRatio,
		PARTICLE_RADIUS:2,
		TRI_MAX:325,
    ...
    var _methods = {
        ...
        _render:function(delta){
            ...
            var DRAW_FILL = _consts.DRAW_FILL;
            var DRAW_PARTICLES = _consts.DRAW_PARTICLES;
            ...
            if (triTime > _consts.TRI_INTERVAL){
				...
				colorTime += triTime;
				if (_consts.COLORS.length && colorTime > _consts.COLOR_TIME){ //Cycle
					colorTime = 0;
            ...
            //Render particles
            if (DRAW_PARTICLES){
				var PARTICLE_RADIUS = _consts.PARTICLE_RADIUS;
				...
				for (var i = 0; i < particleNum; i++){
					...
					context.beginPath();
					context.arc(particle[0], particle[1], PARTICLE_RADIUS, 0, twoPi);
					context.closePath();
